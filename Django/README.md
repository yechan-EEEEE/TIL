...
- ORM, QuerySey API 사용하는 이유:
    1. 데이터베이스 시스템 간에 언어가 달라도 일관된 방식으로 데이터를 다룰 수 있다
    2. 복잡한 SQL 쿼리를 직접 작성하지 않고 Python 코드로 작업이 가능하다
    3. 데이터베이스 테이블을 Python 객체로 다루기 때문에 객체 지향 프로그래밍의 이점을 활용할 수 있다
- Query
...
- Django shell에서 CRUD 사용하기
    - CRUD: Create(저장), Read(조회), Update(갱신), Delete(삭제)
        - Create의 3가지 방법
            ...
            2. article = Article(title='second', content='django'): save를 해야 저장됨
            3. Article.objects.create(title='third', content='django') 바로 저장됨
        - Read의 대표적인 조회 메서드
            1. Return new QuerySets(QuerySet으로 반환)
                - all(): 전체 데이터 조회
                - filter(매개변수): 주어진 매개변수와 일치하는 객체를 포함하는 Queryset 반환
            2. Do not return QuerySets(인스턴스로 반환)
                - get(매개변수): 주어진 매개변수와 일치하는 객체를 반환, 딱 하나만  
                get() 특징: 객체를 찾을 수 없으면 DoesNotExist 예외를 발생시키고, 둘 이상이 있으면 MultipleObjectsReturned  
                위와 같은 특징 때문에 primary key와 같이 고유성을 보장하는 조회에서 사용해야함
        - Update: 인스턴스 변수를 변경 후 save 메서드 호출  
        ![업데이트](img/queryset_api_update.jpg)
        - Delete: 삭제하려는 데이터 조회 후 delete 메서드 호출  
        ![삭제](img/queryset_api_delete.jpg)
- ORM view 함수에서 CRUD 사용하기
    - 2가지 Read
        1. 전체 게시글 조회: ![1](img/queryset_api_createinviews.jpg)
    - Create에 필요한 view 함수
        1. new: 사용자 입력 데이터를 받을 페이지를 렌더링
        2. create: 사용자가 입력한 요청 데이터를 받아 DB에 저장
            ```bash
            - HTTP request mdthods: 데이터에 대해 수행을 원하는 작업(행동)을 나타내는 것
                - 서버에게 원하는 작업의 종류를 알려주는 역할
            - 클라이언트가 웹 서버에 특정 동작을 요청하기 위해 사용하는 표준 명령어
            - GET Method: 서버로부터 데이터를 요청하고 받아오는 데 사용
                1. 데이터 전송: URL의 쿼리 문자열(Query String)을 통해 데이터를 전송(articles/create/? 뒤에 나옴)
                2. 데이터 제한: URL 길이에 제한이 있어 대량의 데이터 전송에는 적합하지 않음
                3. 브라우저 히스토리: 요청 URL이 브라우저 히스토리에 남음
                4. 캐싱: 브라우저는 GET요청의 응답을 로컬에 저장할 수 있음, 동일한 요청을 할 때 저장된 결과를 사용해 로딩 시간을 단축
                예시: 검색 쿼리 전송, 웹 페이지 요청, API에서 데이터 조회
            - POST Method: 서버에 데이터를 제출하여 리소스를 변경(생성, 수정, 삭제)하는 데 사용
                ...
                4. 캐싱: 기본적으로 캐시할 수 없음, 일반적으로 서버의 상태를 변경하는 작업을 수행하기 때문
                예시: 로그인 정보 제출, 파일 업로드, 새 데이터 생성, API에서 데이터 변경 요청
            - CSRF(Cross-Site-Request-Forgery): 사용자가 자기도 모르게 공격자가 의도한 행동을 해 웹 페이지를 공격하게 만드는 방법
                - csrf_token: 요청 시 토근 값도 함께 서버로 전송될 수 있도록 하는 것(POST Method를 사용할 때는 필수),
                겉모습이 똑같은 위조 사이트나 정상적이지 않은 요청에 대한 방어 수단(요청 데이터 -> 게시글 작성 > 요청 데이터+인증 토큰 -> 게시글 작성)
                - GET은 단순 조회를 하지만 POST는 특정 리소스를 변경하는 기술적인 부분을 가지고 있어서 필요
                - 작성하지 않을 시 403 Forbidden(요청이 전달 됐지만, 권한 때문에 거절됐다) 응답이 나옴:(HTTP response status code)
            - HTTP response status code: 서버가 클라이언트의 요청에 대한 처리 결과를 나타내는 3자리 숫자
                1. 요청 처리 결과를 명확히 전달
                2. 문제 발생 시 디버깅에 도움
                3. 웹 애플리케이션의 동작을 제어하는 데 사용
            - redirect() 함수: 동작 후 기존 페이지로 보내는 함수(인자에 작성된 주소로 다시 요청을 보내도록 하는 함수)
            ```
            ![동작원리](img/redirect_동작원리.jpg)
    - Update에 필요한 view 함수
        1. edit
        2. update
